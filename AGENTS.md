...
- Linux desktop notifications depend on a graphical session and use `notify-send` actions plus `xdg-open` for clickable update links, falling back to `beeep`/D-Bus/`notify-send`/`kdialog` when clickable actions are unavailable.
