...
func updateTableWidth(t table.Model, totalWidth int) table.Model {
	...
	tableWidth := totalWidth - baseStyle.GetHorizontalFrameSize() - tableHorizontalFrameSize - terminalRightMargin
	availableWidth := tableWidth - tableFrameWidth
	...
	for i, col := range columns {
		...
		updatedColumns[i] = table.Column{Title: col.Title}
	}

	const minimumStatusWidth = 6
	const minimumPathWidth = 4
	statusIndex := len(columnWidths) - 1
	pathIndex := statusIndex - 1
	needed := minimumStatusWidth - columnWidths[statusIndex]
	transferable := columnWidths[pathIndex] - minimumPathWidth
	if needed > 0 && transferable > 0 {
		if needed > transferable {
			needed = transferable
		}
		columnWidths[pathIndex] -= needed
		columnWidths[statusIndex] += needed
	}
	...
	// The table width includes cell padding; individual column widths do not.
...
func (m model) View() string {
	...
	renderWidth := m.windowWidth - terminalRightMargin
	if renderWidth < 0 {
		renderWidth = 0
	}
	tableContentWidth := renderWidth - baseStyle.GetHorizontalFrameSize()
...
func (m model) tableRows() []table.Row {
	...
	for i, repo := range m.repos {
		rows = append(rows, table.Row{
			...
			m.branchText(i),
			repo.DefaultBranch,
			fsutil.CompactHomePath(repo.Path),
			m.statusText(i),
		})
	}
	return rows
